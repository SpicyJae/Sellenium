 ...
 * End day : Current
 */
public class TestCase04 extends JunitMain {
	...
	public void run() throws Exception{
		...
		try {
			...
			/*
			 * Start date: Current date & End date : Current date
			 */
			Calendar currentStartTimeCal = Calendar.getInstance();
			Calendar currentEndTimeCal = Calendar.getInstance();
			currentStartTimeCal.set(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH), 0, 0, 0);
			...
			for (WebElement vehicleList : vehicleLists) {
				...
				if (!vehicleElementList.get(ALERT_INDEX).getText().equals("0")) {
					...
					/*
					 * Name check between Alert popup and vehicle tab
					 */
					//Comment
//					collector.checkThat( vehicleElementList.get(VEHICLE_INDEX).getText() + ": License number" , vehicleElementList.get(LICENSE_NO_INDEX).getText().trim() , is(driver.findElement(Alert_PopUpNameLocator).getText().trim()));
					 ...
					 * Alert list # check
					 */
					List<WebElement> alertLists = driver.findElements(alertListLocator);
					...
					for (WebElement alertList : alertLists) {
						 ...
						 */
						collector.checkThat( vehicleElementList.get(VEHICLE_INDEX).getText() + ": Alert's Time out of the range", (currentStartTimeCal.getTime().getTime() - alertTime.getTime() < 0) && (currentEndTimeCal.getTime().getTime() - alertTime.getTime() > 0) , is(true));
					}
