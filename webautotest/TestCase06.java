 ...
 * Start day : Current - 30 days
 * End day : Current
 */
public class TestCase06 extends JunitMain {
	...
	public void run() throws Exception{
		try {
			...
			Time tm = new Time();
			/*
			 * Change the start date = current date - 30
			...
			if( Calendar.getInstance().get(Calendar.DAY_OF_MONTH) >= 28 ){
				dateRange.ChangeStartDateRange(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH)-1, 28);
				...
				currentStartTimeCal.set(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH) - 1 , 28 ,0, 0, 0);
				...
				dateRange.ChangeStartDateRange(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH)-1, Calendar.getInstance().get(Calendar.DAY_OF_MONTH));
				dateRange.ChangeEndDateRange(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH));
				currentStartTimeCal.set(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH) - 1 , Calendar.getInstance().get(Calendar.DAY_OF_MONTH),0, 0, 0);
				currentEndTimeCal.set(Calendar.getInstance().get(Calendar.YEAR),Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH), 23,	59, 59);
			}
			/*
			...
			pageLoad.stop();
			collector.checkThat( "Page Loading Time is too long" , pageLoad.getTime() < 8000, is(true));
			...
			for (WebElement vehicleList : vehicleLists) {
				...
				/*
				 * If the HOURS column have "00:00:00" value, we don't need to
				 * check trip detail
				 */
				...
				collector.checkThat( vehicleElementList.get(VEHICLE_INDEX).getText() + ": Trip's Fuel" , (Math.abs(detFuel - Double.parseDouble(vehicleElementList.get(FUEL_INDEX).getText())) <= THRES_HOLD) , is(true));
				if(detFuel!=0){
